package com.example.tiktolkcommentdialog.adapter;
...
import com.example.tiktolkcommentdialog.util.TimeUtils;
import com.example.tiktolkcommentdialog.widget.TextClickSpans;
import com.example.tiktolkcommentdialog.widget.TextMovementMethods;
...
public class CommentDialogMutiAdapter extends BaseMultiItemQuickAdapter<MultiItemEntity, BaseViewHolder> {
    ...
    public CommentDialogMutiAdapter(List list) {
        ...
        //pos&(~pos+1); 将二进制位中最右侧的1提取出来
    }

    @Override
    protected void convert(@NonNull BaseViewHolder helper, MultiItemEntity item) {
        Log.e("convert","itemType = "+item.getItemType());
    ...
    private void bindCommentParent(BaseViewHolder helper, FirstLevelBean item) {
        ...
        Glide.with(mContext).load(item.getHeadInt()).into(iv_header);
